...
- **Phase 1**: 未着手（プロジェクトセットアップ・基盤構築）
- **Phase 2**: 未着手（型定義・コアロジック実装）
- **Phase 3**: 未着手（UI コンポーネント実装）
- **Phase 4**: 未着手（履歴管理機能実装）
- **Phase 5**: 未着手（メインページ統合・レイアウト実装）
- **Phase 6**: 未着手（Cloud Run デプロイ準備）
- **Phase 7**: 未着手（検証・最適化・デプロイ）

### リスク管理

| リスク                                   | 影響度 | 発生確率 | 対策                                                |
| ---------------------------------------- | ------ | -------- | --------------------------------------------------- |
| math.js バンドルサイズ超過（>500KB）     | 高     | 中       | tree-shaking最適化、必要に応じてexpr-evalへ移行     |
| ブラウザ互換性問題（Safari等）           | 中     | 低       | polyfill追加、代替実装検討                          |
| Cloud Run コールドスタート遅延           | 中     | 中       | min-instances設定検討（コスト増加注意）             |
| localStorage容量制限（履歴100件以上）    | 低     | 低       | 古い履歴の自動削除実装済み                          |
| アクセシビリティ基準未達                 | 中     | 低       | 初期段階からARIA属性・セマンティックHTML使用        |
| ターミナルデザインの可読性問題（色対比） | 中     | 低       | WCAG AA準拠カラーパレット使用、コントラスト比確認済 |
...
- [ ] axe DevTools監査合格（WCAG AA）
- [ ] キーボードナビゲーション動作確認（Tab, Enter, Escape等）
...
- [ ] ターミナルテーマ統一確認（配色・フォント・エフェクト）
- [ ] ブラウザコンソールエラー・警告なし
- [ ] ホバー・フォーカス・アクティブ状態の適切な表示
- [ ] ダークモード固定確認
